...
<?php 
    $no=0;
    foreach ($iklan_anda->result() as $key) {
        $no++;
        ...
        echo "
            <tr>
                ...
                <td>
                    ...
                    <button class='btn btn-xs btn-danger btn-block'  onclick='hapus($key->id_iklan,$key->status)'>Hapus</button>
                </td>
        ...
        <div class="modal-body">
            <form id="form_modal">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2" name="provinsi" id="provinsiIklan" style="width:100%" required>
                ...
                <div class="form-group">
                    ...
                    <select class="select2 form-control" name="kota_kab" id="kota_kab_iklan"  style="width: 100%">
                    </select>
                </div>
...
function hapus(id,val)
{
    if(confirm("Anda Yakin?"))
    {
        $.post("<?php echo base_url()?>index.php/welcome/hapus/",{id:id,val:val},function(){
            iklan_anda();
        })
    }
}
...
******/
    ...
    $("#info").html("");
    $.get("<?php echo base_url()?>index.php/welcome/iklan_by_id/"+id,function(e){
        console.log(e[0]);
        var x = e[0];
        ...
        */
        ...
        $("#provinsiIklan").val(x.provinsi).trigger('change');
        $('#kota_kab_iklan').append(new Option(x.nama_kota, x.kota_kab, false, false)).trigger('change');
...
$("#form_modal").on("submit",function(){
    ...
    $.ajax({
            url: "<?php echo base_url()?>index.php/welcome/simpan_iklan",
            type: "POST",
            contentType: false,
            processData:false,
...
$("#provinsiIklan").on("change",function(){
    ...
    $.get("<?php echo base_url()?>index.php/welcome/ambil_kab/"+$(this).val(),function(e){
        ...
        if($("#id_iklan").val() == "")
        {
            $("#kota_kab_iklan").empty();    
        }
        
        $.each(e,function(a,b){
            console.log(b.name);
            $("#kota_kab_iklan").append("<option value='"+b.id+"'>"+b.name+"</option>");
        })
    })
})
